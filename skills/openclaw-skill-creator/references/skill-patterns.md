...
## Pattern E: Slash Command Skills (OpenClaw)

**Used for:** Quick actions that bypass the model and dispatch directly to a tool.
...
The user types `/quick-lint src/main.py` and the tool receives `{ command: "src/main.py", commandName: "quick-lint", skillName: "quick-lint" }`. No model inference occurs — this is instant.

**Key techniques:**
- Set `command-dispatch: tool` + `command-tool` for direct dispatch
- Set `disable-model-invocation: true` to keep it out of the model prompt (saves tokens)
- Keep `user-invocable: true` so it appears as a slash command
- The SKILL.md body is irrelevant for direct dispatch — the tool handles everything

**When to use:** Repetitive tool calls that don't need model reasoning. Linters, formatters, quick searches, status checks.

## Pattern F: Gated Skills (OpenClaw)

**Used for:** Skills that should only appear when their dependencies are available.

```yaml
---
name: image-gen
description: Generate images using Stable Diffusion via ComfyUI
metadata: {"openclaw": {"requires": {"bins": ["comfyui"], "env": ["COMFYUI_URL"]}, "primaryEnv": "COMFYUI_URL", "os": ["darwin", "linux"], "emoji": "🎨", "install": [{"id": "brew", "kind": "brew", "formula": "comfyui", "bins": ["comfyui"], "label": "Install ComfyUI (brew)"}]}}
---
```

**Key techniques:**
- Use `requires.bins` for CLI tools the skill depends on
- Use `requires.env` for API keys — combine with `primaryEnv` so users can set it via `skills.entries.<key>.apiKey`
- Use `os` to restrict to platforms where the tool actually works
- Provide `install` specs so the macOS Skills UI can auto-install dependencies
- The skill is invisible to the model when requirements aren't met — no wasted tokens

**When to use:** Any skill that depends on external binaries, API keys, or specific platforms. Prevents confusing errors when the agent tries to use unavailable tools.

## Pattern G: Context-Aware Tool Selection
...
## Pattern H: Domain-Specific Intelligence
