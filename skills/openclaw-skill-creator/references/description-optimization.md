...
**OpenClaw-specific mechanics:** To run a trigger eval in OpenClaw, you need the eval agent profiles set up via `scripts/trigger_eval/setup_agents.sh <skill-name>`. Then for each query, spawn a subagent at `eval-with-<skill-name>` via `sessions_spawn` and check whether it consulted the skill using `scripts/trigger_eval/orchestrator.py`'s `detect_skill_consultation()` (which scans `sessions_history` for a `read` tool call matching `<skill-name>/SKILL.md`). Run each query 3× to account for nondeterminism. After editing the description, restart the OpenClaw gateway (`openclaw gateway restart`) so the new snapshot is loaded — the previous description otherwise stays cached for the existing session.
